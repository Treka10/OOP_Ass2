...
template <typename T>
class Numerical_RandPlayer : public RandomPlayer<T>{
public:
    Numerical_RandPlayer (T symbol);
    void getmove(int &x, int &y) ;
};
...
bool Numerical_Board<T>::is_win() {
    ...
    if ((this->board[0][0] + this->board[1][1] + this->board[2][2] ==15) && (this->board[0][0] != 0 && this->board[1][1] != 0 && this->board[2][2] !=0) ||
        ((this->board[0][2] + this->board[1][1] + this->board[2][0] == 15)&& (this->board[2][0] != 0 && this->board[1][1] != 0 && this->board[0][2] !=0))) {
        return true;
    }
...
void Numerical_Player<T>::getmove(int& x, int& y) {
    ...
    while (!((number % 2 == 1 && this->symbol % 2 == 1) || (number % 2 == 0 && this->symbol % 2 == 0) )|| (number>9 || number <1)) {
        cout << "Repeated Numbers are not valid,Please enter your move (x, y) and number (1, 3, 5, 7, 9 for Player 1 or 2, 4, 6, 8 for Player 2): ";
        cin >> x >> y >> number;
    }
    this->symbol = number;
}
...
template <typename T>
Numerical_RandPlayer<T>::Numerical_RandPlayer(T symbol) : RandomPlayer<T>(symbol) {
    this->dimension = 3;
...
template <typename T>
void Numerical_RandPlayer<T>::getmove(int& x, int& y) {
